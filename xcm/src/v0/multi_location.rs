...
mod tests {
    ...
    fn prepend_with_works() {
        ...
        assert_eq!(
            m,
            X5(Parachain(100), OnlyChild, OnlyChild, OnlyChild, OnlyChild)
        );
        ...
        assert_eq!(
            m,
            X8(Parent, Parent, Parent, Parent, Parent, Parent, Parent, Parent)
        );

        let mut m = X1(Parent);
        let prefix = X8(
            Parent, Parent, Parent, Parent, OnlyChild, Parent, Parent, Parent,
        );
        assert_eq!(m.prepend_with(prefix.clone()), Ok(()));
        assert_eq!(
            m,
            X7(Parent, Parent, Parent, Parent, Parent, Parent, Parent)
        );
    }
