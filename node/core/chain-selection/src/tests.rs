...
use polkadot_subsystem::{jaeger, ActivatedLeaf, ActiveLeavesUpdate, LeafStatus};
...
async fn import_all_blocks_into(
    ...
    assert!(
        blocks.len() > 1,
        "gap only makes sense if importing multiple blocks"
    );
...
fn assert_backend_contains<'a>(
    ...
    for header in headers {
        ...
        assert!(
            backend
                .load_blocks_by_number(header.number)
                .unwrap()
                .contains(&hash),
...
async fn assert_leaves_query(virtual_overseer: &mut VirtualOverseer, leaves: Vec<Hash>) {
    assert!(
        !leaves.is_empty(),
        "empty leaves impossible. answer finalized query"
    );
