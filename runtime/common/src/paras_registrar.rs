...
impl<T: Config> Registrar for Pallet<T> {
    ...
    fn worst_head_data() -> HeadData {
        ...
        assert!(
            max_head_size > 0,
            "max_head_data can't be zero for generating worst head data."
        );
    ...
    fn worst_validation_code() -> ValidationCode {
        ...
        assert!(
            max_code_size > 0,
            "max_code_size can't be zero for generating worst code data."
        );
...
impl<T: Config> Pallet<T> {
    ...
    fn validate_onboarding_data(
        ...
        let config = configuration::Pallet::<T>::config();
            ...
            validation_code.0.len() <= config.max_code_size as usize,
